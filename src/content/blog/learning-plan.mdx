...
pubDate: 2024-09-16
description: 'A post that will get updated over time listing my readings/learning goals, hopefully correctly divided by subject.'
---
## Structure
...
flowchart TD;
    ...
    B-->CCom[Circuits & Electronics];
    CCom-->DCom[Computation Structures];
    DCom-->ECom[Software Construction];
    ECom-->FCom[Computer Networking];
    FCom-->GCom[Computer Systems Engineering];
    GCom-->HCom[Databases];
    HCom-->ICom[Distributed Systems];
    ICom-->JCom[Compilers];

    JCom --> |?| JQu[Quantum Computing];
    ...
    CPh-->DPh[Mult. Calc.];
    DPh-->EPh[Physics III];
    EPh-->FPh[Susskind Classical Mech];
    FPh-->GPh[Susskind Others];
    GPh-->|?| HPh[Statistical Physics];
    HPh-->|?| IPh[Differential Forms];
    HPh-->|?| JQu[Quantum Computing];
    HPh-->|?| KPh[Non-linear Dynamics & Chaos];
    HPh-->|?| LPh[Real Analysis Refresher];
```
